...
export default function DailyStressChart({ data }: Props) {
  const filtered = data.filter((d) => d.stressLevel != null);

  if (filtered.length === 0) {
    return (
      <EmptyState title="Stress quotidien" message="Aucune donnée de stress" />
    );
  }
  ...
  const values = filtered.map((d) => d.stressLevel!);
  ...
  const getStressColor = (value: number) => {
    if (value <= 25) return "#22c55e"; // green - low/rest
    if (value <= 50) return "#3b82f6"; // blue - low/medium
    if (value <= 75) return "#f97316"; // orange - medium/high
    ...
    <ChartContainer
      ...
      legend={
        <ChartLegend
          ...
          items={[
            { label: "Quotidien", color: "bg-blue-300", shape: "line" },
            { label: "Moyenne 7j", color: "bg-blue-500", shape: "line" },
          ]}
        />
      }
    >
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full">
        ...
        <line
          ...
          strokeWidth="1"
        />
        <text x={pad.left + chartW + 2} y={avgY + 3} fontSize="8" className="fill-blue-500">
          {Math.round(avg)}
        </text>

        {/* Raw data line */}
        <path d={polylinePath(points)} fill="none" stroke="#93c5fd" strokeWidth="1.5" />
        ...
        {filtered.map((d, i) => (
          <circle
            ...
            cy={points[i].y}
            r="2.5"
            fill={getStressColor(d.stressLevel!)}
          />
        ))}

        {/* Moving average */}
        {maPoints.length > 1 && (
          <path d={polylinePath(maPoints)} fill="none" stroke="#3b82f6" strokeWidth="2" />
        )}
