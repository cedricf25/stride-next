...
import { fetchHrvAnalysis, generateHrvAnalysis, type HealthAnalysisResult } from "@/actions/gemini-health";
...
export default function HrvAiAnalysis({ initialAnalysis }: Props) {
  const [analysis, setAnalysis] = useState(initialAnalysis);
  const [loading, setLoading] = useState(false);

  const handleGenerate = useCallback(async () => {
    setLoading(true);
    const result = await generateHrvAnalysis();
    setAnalysis(result);
    setLoading(false);
  }, []);
    ...
    <section className="mt-8">
      ...
      {analysis.analysis && !loading && (
        <>
          <Card padding="sm" className="mb-4 flex items-center gap-4 text-sm text-gray-600">
            ...
            <span>{analysis.dataPointsCount} mesures</span>
            {analysis.createdAt && (
              <>
                <span className="text-gray-400">•</span>
      ...
      {!analysis.analysis && !loading && !analysis.error && (
        <Card padding="md" className="text-center text-gray-500">
          ...
          <p>Cliquez sur "Générer l'analyse" pour obtenir une analyse IA de votre HRV.</p>
