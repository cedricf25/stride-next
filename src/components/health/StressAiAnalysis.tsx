...
export default function StressAiAnalysis({ initialAnalysis }: Props) {
  const [analysis, setAnalysis] = useState(initialAnalysis);
  const [loading, setLoading] = useState(false);

  const handleGenerate = useCallback(async () => {
    setLoading(true);
    const result = await generateStressAnalysis();
    setAnalysis(result);
    setLoading(false);
  }, []);
    ...
    <section className="mt-8">
      ...
      {!analysis.analysis && !loading && !analysis.error && (
        <Card padding="md" className="text-center text-gray-500">
          ...
          <p>Cliquez sur "Générer l'analyse" pour obtenir une analyse IA de votre stress nocturne.</p>
